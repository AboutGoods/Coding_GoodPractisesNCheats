...
Means that the entity before **?** can be nil. You are not sure at all. It is useful when you are dealing with asynchronous task (Network, processing, image management, file etc.) you are telling to your program that this var is maybe here, maybe not. And **this is YOUR responsability to test if entity is nil before using it !**
...
It is probably the most dangerous sign in Swift. You can only use is on a optional variable, marked with "?".

Using "!" you tell to the compiler that a variable that is declared optional, is filled at this time. It avoid making test for nil.

You have to use it very prudently as it will most of the time make your app crash, maybe. You are not sure.

Even if you are absolutely sure that your optional will NOT be nil, you are actually wrong. The OS can deallocate some memory, maybe network will be unavailable precisely just before your `myOptionalVar!.callMethod()` and you get a nice crash.
...
**When your variable is normal (no "!" or "?")**

  When Xcode don't bother you with ? or ! that means you can be sure that your entity is not nil at this time, because your entity is not declared as optional or already instanciated when class is instanciated.
